    ...
    <div style={{ minHeight:'100vh', background:'#0B1F3A', fontFamily:"'IBM Plex Sans', sans-serif" }}>
      ...
      <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, height:'64px', background:'rgba(11,31,58,0.95)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(201,168,76,0.15)', display:'flex', alignItems:'center', padding:'0 32px', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', gap:'10px' }}>
          {editando ? (
            <>
              <button onClick={() => { setEditando(false); setForm(lider) }} style={{ background:'transparent', border:'1px solid #1C3558', borderRadius:'8px', color:'#8FA4C0', fontSize:'13px', padding:'8px 18px', cursor:'pointer', fontFamily:"'IBM Plex Sans', sans-serif" }}>Cancelar</button>
              <button onClick={handleSalvar} disabled={salvando} style={{ background: salvando ? 'rgba(201,168,76,0.5)' : 'linear-gradient(135deg, #E8C87A, #A07830)', border:'none', borderRadius:'8px', color:'#0B1F3A', fontSize:'13px', fontWeight:600, padding:'8px 22px', cursor:'pointer', fontFamily:"'IBM Plex Sans', sans-serif" }}>
      ...
      <main style={{ paddingTop:'88px', padding:'88px 32px 60px', position:'relative', zIndex:1, maxWidth:'860px', margin:'0 auto' }}>

        {/* HEADER DO LÍDER */}
        <div style={{ display:'flex', alignItems:'flex-start', gap:'24px', marginBottom:'32px', background:'#0F2040', border:'1px solid #1C3558', borderRadius:'12px', padding:'28px' }}>
          <div style={{ width:'72px', height:'72px', borderRadius:'50%', background:'rgba(201,168,76,0.15)', border:'2px solid rgba(201,168,76,0.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px', fontWeight:700, color:'#C9A84C', flexShrink:0 }}>
            {lider.nome.charAt(0).toUpperCase()}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'6px' }}>
              <h1 style={{ fontFamily:"'Playfair Display', serif", fontSize:'26px', fontWeight:800, color:'#FFFFFF' }}>{lider.nome}</h1>
            ...
            <div style={{ display:'flex', gap:'24px', flexWrap:'wrap' as const }}>
              <div style={{ textAlign:'center' as const }}>
                <div style={{ fontFamily:"'Playfair Display', serif", fontSize:'28px', fontWeight:800, color:'#C9A84C' }}>{totalApoiadores}</div>
              ...
              {lider.zona_eleitoral && <div style={{ textAlign:'center' as const }}>
                <div style={{ fontFamily:"'Playfair Display', serif", fontSize:'28px', fontWeight:800, color:'#6ba3d6' }}>{lider.zona_eleitoral}</div>
              ...
              {lider.secao_eleitoral && <div style={{ textAlign:'center' as const }}>
                <div style={{ fontFamily:"'Playfair Display', serif", fontSize:'28px', fontWeight:800, color:'#5eead4' }}>{lider.secao_eleitoral}</div>
                <div style={{ fontSize:'11px', color:'#8FA4C0', textTransform:'uppercase' as const, letterSpacing:'1px' }}>Seção</div>
              </div>}
            </div>
          </div>
          <div style={{ display:'flex', flexDirection:'column' as const, gap:'8px' }}>
            <button onClick={() => router.push(`/dashboard/apoiadores?lider=${lider.id}`)} style={{ background:'rgba(201,168,76,0.1)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:'8px', color:'#C9A84C', fontSize:'13px', padding:'8px 16px', cursor:'pointer', fontFamily:"'IBM Plex Sans', sans-serif", whiteSpace:'nowrap' as const }}>
              👥 Ver apoiadores
            </button>
            <button onClick={toggleAtivo} style={{ background: lider.ativo ? 'rgba(192,57,43,0.1)' : 'rgba(34,197,94,0.1)', border:`1px solid ${lider.ativo ? 'rgba(192,57,43,0.2)' : 'rgba(34,197,94,0.2)'}`, borderRadius:'8px', color: lider.ativo ? '#e74c3c' : '#22c55e', fontSize:'13px', padding:'8px 16px', cursor:'pointer', fontFamily:"'IBM Plex Sans', sans-serif", whiteSpace:'nowrap' as const }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px' }}>
          ...
          <div style={{ background:'#0F2040', border:'1px solid #1C3558', borderRadius:'12px', padding:'24px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column' as const, gap:'14px' }}>
              {[['CEP','cep'],['Endereço','endereco'],['Bairro','bairro'],['Cidade','cidade'],['Estado','estado']].map(([label, campo]) => (
                <div key={campo}>
                  <div style={labelStyle}>{label}</div>
                  {editando
                    ? <input value={form[campo as keyof typeof form] as string || ''} onChange={e => handleChange(campo, e.target.value)} style={{ ...inputStyle, marginTop:'4px' }} onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor='#1C3558'} />
          ...
          <div style={{ background:'#0F2040', border:'1px solid #1C3558', borderRadius:'12px', padding:'24px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column' as const, gap:'14px' }}>
              {[['Zona Eleitoral','zona_eleitoral'],['Seção Eleitoral','secao_eleitoral']].map(([label, campo]) => (
                <div key={campo}>
                  <div style={labelStyle}>{label}</div>
                  {editando
                    ? <input value={form[campo as keyof typeof form] as string || ''} onChange={e => handleChange(campo, e.target.value)} style={{ ...inputStyle, marginTop:'4px' }} onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor='#1C3558'} />
