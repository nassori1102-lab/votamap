    ...
    <div style={{ minHeight:'100vh', background:'#0B1F3A', fontFamily:"'IBM Plex Sans', sans-serif" }}>
      ...
      <main style={{ paddingTop:'88px', padding:'88px 32px 60px', position:'relative', zIndex:1, maxWidth:'760px', margin:'0 auto' }}>
        <div style={{ marginBottom:'32px' }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display', serif", fontSize:'32px', fontWeight:800, color:'#FFFFFF' }}>Cadastrar Apoiador</h1>
        </div>
        ...
        <form onSubmit={handleSalvar} style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                {[1,2,3,4,5].map(n => (
                  <button key={n} type="button" onClick={() => handleChange('engajamento', String(n))}
                    style={{ width:'40px', height:'40px', borderRadius:'8px', border:`1px solid ${parseInt(form.engajamento) >= n ? 'rgba(201,168,76,0.4)' : '#1C3558'}`, background: parseInt(form.engajamento) >= n ? 'rgba(201,168,76,0.15)' : 'transparent', color: parseInt(form.engajamento) >= n ? '#C9A84C' : '#8FA4C0', fontSize:'18px', cursor:'pointer', transition:'all .15s' }}>
                    ★
                  </button>
                ))}
          ...
          {/* CONSENTIMENTO LGPD */}
          <div style={{ background:'rgba(201,168,76,0.05)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:'12px', padding:'24px' }}>
